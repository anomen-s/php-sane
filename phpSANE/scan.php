...
if($geometry_l >= 0 && $geometry_l <= 216) $cmd_geometry_l=" -l ".$geometry_l."mm"; else {
...
if($geometry_x >= 0 && $geometry_x <= 216) $cmd_geometry_x=" -x ".$geometry_x."mm"; else {
...
if($resolution >= 5 && $resolution <= 9600) $cmd_resolution=" --resolution ".$resolution."dpi"; else {
$lang[$lang_id][18]="<font style=\"color:red;\">".$lang[$lang_id][18]."</font>";
scan_error(&$scan_ausgabe, &$style_bold, &$style_color, &$style_center, &$error_input, $lang_error);
}
}

if($negative == "yes") $cmd_negative=" "; else $cmd_negative="";
if($quality_cal == "yes") $cmd_quality_cal=" "; else $cmd_quality_cal="";
//if($brightness) {
...
echo "window.open(\"save.php?file=".$file_scan."&lang_id=".$lang_id."\",\"_blank\", \"width=400,height=500,left=320,top=200,scrollbars=yes,location=no,status=no,menubar=no\");\n";
//echo "command used was: ".$cmd_scan;
echo "</script>\n";
} else {
$cmd_scan=$cmd_scan." > ".$file_scan;
$scan_yes=`$cmd_scan`;
echo "<script language=\"JavaScript\" type=\"text/javascript\">\n";
echo "window.open(\"save.php?file=".$file_scan."&lang_id=".$lang_id."\",\"_blank\", \"width=400,height=300,left=320,top=200,scrollbars=yes,location=no,status=no,menubar=no\");\n";
echo "</script>\n";
}
}
...
$text=$file.".txt";
echo "<script language=\"JavaScript\" type=\"text/javascript\">\n";
echo "window.open(\"save.php?file=".$text."&lang_id=".$lang_id."\",\"_blank\", \"width=400,height=300,left=320,top=200,scrollbars=yes,location=no,status=no,menubar=no\");\n";
echo "</script>\n";
}
}
